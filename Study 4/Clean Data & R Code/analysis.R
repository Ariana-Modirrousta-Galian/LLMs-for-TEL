...
# write.csv(part3 %>% select(starts_with("SAQ")),"SAQ_participant_answers.csv", row.names = FALSE)
...
# })
# practice = data.frame(part3_id = saq_part2$part2_id,
#                       practice_prop = rowMeans(saq_items))
# export_data = part3[, c("part3_id", "Group", "prop_correct")]
# export_data$practice_prop = practice$practice_prop[match(export_data$part3_id, practice$part3_id)]
# write.csv(export_data, "data_AMG_2.csv", row.names = FALSE)
...
# Correlation between practice and final test performance
...
final = part3 %>% select(part3_id, prop_correct)
pt_ft = inner_join(practice, final, by = "part3_id")
cor.test(pt_ft$practice, pt_ft$prop_correct)
