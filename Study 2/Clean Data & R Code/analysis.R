...
# export_data = part3[, c("Participant.ID", "Group", "prop_correct")]
# practice = part2[part2$QuestionCount == 20, ]
# export_data$practice_prop = as.numeric(practice$SC0[match(export_data$Participant.ID, practice$Participant.ID)]) / 20
# write.csv(export_data, "data_JZ.csv", row.names = FALSE)
...
practice = part2 %>%
  filter(QuestionCount == 20) %>%
  transmute(Participant.ID, practice = as.numeric(SC0))
final = part3 %>%
  transmute(Participant.ID, final = SC0)
pt_ft = inner_join(practice, final, by = "Participant.ID")
