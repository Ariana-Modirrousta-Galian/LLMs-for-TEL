...
class_exams = read_excel("classexams.xlsx")
...
# Remove empty trailing rows and calculate the average class exam grade
# across all five class exams (Weeks 2, 4, 6, 8, and 10), ignoring any missing exams
class_exams = class_exams |>
  filter(!is.na(Candidate)) |>
  distinct(Candidate, .keep_all = TRUE) |>
  mutate(AVERAGE = rowMeans(across(c(Week2_Percent, Week4_Percent, Week6_Percent, Week8_Percent, Week10_Percent)), na.rm = TRUE))
...
# P07, P18, P09, and P13 did not have an average class exam grade

# Linear regression 
h1_class_exams_model = lm(AVERAGE ~ proportion_tests_completed, data = h1_class_exams)
summary(h1_class_exams_model)
# Effect is in the predicted direction, so one-tailed p-value = .007
...
# Effect is in the predicted direction, so one-tailed p-value = .006
...
# P07, P18, P13, and P09 did not have a final exam grade
...
# T-test vs. mid-point
t.test(h3_end$ai_perception_end, mu = 4, alternative = "greater")
cohensD(h3_end$ai_perception_end, mu = 4)
ttestBF(h3_end$ai_perception_end, mu = 4, nullInterval = c(0, Inf))
